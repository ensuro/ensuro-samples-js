...
async function resolvePolicyCommand(argv) {
  ...
  const policyData = JSON.parse(fs.readFileSync(argv.policyData));
  const rmAddress =
    argv.rmAddress || policyData.riskModule || policyData.data[11];
  const rm = new ethers.Contract(rmAddress, ABI, signer);
  let tx;
  if (
    argv.result.toLowerCase() == "false" ||
    argv.result.toLowerCase() == "true"
  ) {
    tx = await ensuro.resolvePolicyFullPayout(
      policyData.data,
      argv.result.toLowerCase() == "true",
      rm
    );
  } else {
    tx = await ensuro.resolvePolicy(
      policyData.data,
      Number.parseFloat(argv.result),
      rm
    );
  }
  await tx.wait();
...
async function resolveFDPolicyCommand(argv) {
  const rm = new ethers.Contract(
    argv.rmAddress,
    ensuro.ABIS.FlightDelayRiskModule,
    signer
  );
  let tx;
  let policyId =
    argv.rmAddress + argv.internalId.toString(16).padStart(24, "0");
...
async function computePremium(argv) {
  ...
  if (argv.expiration >= 1672691181) {
    ...
    expiration =
      Math.round(new Date().getTime() / 1000) + argv.expiration * 3600 * 24;
  } else {
    // Relative Timestamp in seconds
    expiration = Math.round(new Date().getTime() / 1000) + argv.expiration;
  }
  const result = ensuro.computePremium(
    rmParams,
    argv.payout,
    argv.lossProb,
    expiration
...
async function printTotalSupply(argv) {
  const etk = new ethers.Contract(
    argv.etkAddress,
    ensuro.getABI("EToken"),
    signer
  );
...
async function quotePolicy(argv) {
  ...
  if (!API_KEY) {
    console.log(
      "Error, API key not defined, must define environment variable QUOTE_API_KEY"
    );
    process.exit(1);
  }
  let expiration;
  if (argv.expiration.match(RegExp("^1[0-9]{9}$"))) {
    ...
    // Relative Timestamp
    expiration =
      Math.round(new Date().getTime() / 1000) + parseInt(argv.expiration);
  } else {
  ...
  let jsonData;
  if (
    argv.jsonData.startsWith("https://") ||
    argv.jsonData.startsWith("http://")
  ) {
    // Remote JSON file
  ...
  const response = await axios.post(argv.apiEndpoint, jsonParams, {
...
async function newPolicyWebhook(argv) {
  ...
  if (argv.signatureSecret) {
    headers["X-Ensuro-Signature"] = ethers.utils
      .computeHmac(
        "sha256",
    ...
    const contentDigest = ethers.utils
      .sha256(ethers.utils.toUtf8Bytes(requestBody))
      .replace(/^0x/, "");
    const wallet = new ethers.Wallet(argv.eip191SigningKey);
    headers["X-EIP191-Signature"] = await wallet.signMessage(contentDigest);
  } else {
    console.log(
      "Error, you must specify either eip191SigningKey or signatureSecret"
    );
    process.exit(1);
  }
  const response = await axios.post(argv.apiEndpoint, requestBody, {
    headers: headers,
  });
  if (argv.outputFile == "-") {
    console.log("Response: ", response.data);
...
yargs
  .scriptName("ensuro-cli")
  .usage("$0 <cmd> [args]")
  .command(
    "new-policy <policyData> <customer> [--rm <rm-address>]",
    "Create new policy",
    (yargs) => {
      yargs.positional("policyData", {
        type: "string",
        describe:
          "Json file with the data of the policy to be created - See sample-policy-<rmType>.json",
      });
  ...
  .command(
    "quote-policy <apiEndpoint>",
    "Get a signed quote from API",
    (yargs) => {
...
Can be sent as ISO 8601 timestamp, epoch timestamp or integer (seconds relative to now)`,
      ...
      yargs.option("outputFile", {
        type: "string",
        describe:
          "Output file where the policy data will be saved (json). Default stdout",
        default: "-",
      });
    },
    quotePolicy
  ...
  .command(
    "new-policy-webhook <apiEndpoint>",
    "Create a policy using the webhook",
    (yargs) => {
      yargs.positional("apiEndpoint", {
        type: "string",
        describe: `The Quote API endpoint.
...
Can be sent as ISO 8601 timestamp, epoch timestamp or integer (seconds relative to now)`,
      ...
      yargs.option("signatureSecret", {
        type: "string",
        describe:
          "Signing secret to do the HMAC-SHA256 signature of the request body",
        default: null,
      });
      yargs.option("outputFile", {
        type: "string",
  ...
  .command(
    "resolve-policy <policyData> <result>",
    "Resolve policy",
    (yargs) => {
      yargs.positional("policyData", {
        type: "string",
        describe:
          "Json file with the full data of the policy, generated by new-policy",
      });
      yargs.positional("result", {
        type: "string",
        describe:
          'Resolution of the policy. Might be "true" or "false" if full payout or number',
      });
      yargs.option("rmType", {
        describe: "Type of RiskModule",
        default: "SignedQuoteRiskModule",
      ...
      yargs.option("rmAddress", {
        describe:
          "Address of the RiskModule contract (if undefined, took from policyData)",
        type: "string",
        default: undefined,
      });
    },
  ...
  .command(
    "resolve-fd-policy <internalId> [--rm <rm-address>]",
    "Triggers resolution of Flight Delay policy",
    (yargs) => {
  ...
  .command(
    "fetch-rm-params [--rm <rm-address>]",
    "Fetch Risk Module Params",
    (yargs) => {
      yargs.option("rmAddress", {
        describe: "Address of the RiskModule contract ",
        type: "string",
      ...
      yargs.option("outputFile", {
        type: "string",
        describe:
          "Output file where the rm parameters will be saved (json). Default stdout",
        default: "-",
      });
    },
  ...
  .command(
    "compute-premium <payout> <lossProb> <expiration>",
    "Fetch Risk Module Params",
    (yargs) => {
      ...
      yargs.positional("lossProb", {
        describe:
          "Probability of paying the maximum payout (lossProb * payout = expectedLoss)",
        type: "number",
      });
      yargs.positional("expiration", {
        describe:
          "Policy expiration (relative in days if <1000, unix timestamp, else relative in seconds)",
        type: "number",
      });
      yargs.option("rmAddress", {
        describe: "Address of the RiskModule contract ",
  ...
  .command(
    "print-total-supply [--etk-address etk-address]",
    "Print EToken Total Supply",
    (yargs) => {
  ...
  .command(
    "approve [--erc20Address address] [--spender address]",
    "ERC20 Approval",
    (yargs) => {
  ...
  .command(
    ...
    "Ask money to the faucet contract (only testnet)",
    (yargs) => {
      yargs.option("faucetContract", {
        describe:
          "Address of the Faucet contract that implements `tap` function",
        type: "string",
